...
def test_fetch_builds_request_url_and_checksum() -> None:
    ...
    assert "REDACTED" in response.source_url
...
def test_fetch_rejects_unsupported_metric() -> None:
    connector = CensusAcsConnector(api_key="test-key")
...
def test_fetch_rejects_malformed_geography() -> None:
    connector = CensusAcsConnector(api_key="test-key")
...
def test_normalize_builds_observation_with_provenance() -> None:
    connector = CensusAcsConnector(retrieved_at=date(2026, 1, 1), api_key="test-key")
    request = DataRequest(geography="55:43075", metric_ids=("education_attainment",))
    with patch(
        "retirement_engine.connectors.census_acs.urlopen",
...
def test_normalize_reports_html_error_page_as_key_problem() -> None:
    connector = CensusAcsConnector(api_key="test-key")
...
def test_normalize_rejects_unknown_state_fips() -> None:
    connector = CensusAcsConnector(api_key="test-key")
...
def test_validate_accepts_well_formed_observations() -> None:
    connector = CensusAcsConnector(retrieved_at=date(2026, 1, 1), api_key="test-key")
    with patch(
        "retirement_engine.connectors.census_acs.urlopen",
        return_value=_mock_response(SAMPLE_PAYLOAD),
...
def test_validate_flags_negative_estimate() -> None:
    connector = CensusAcsConnector(retrieved_at=date(2026, 1, 1), api_key="test-key")
    payload: list[list[str]] = [
        ["NAME", "DP02_0068PE", "state", "place"],
...
def test_normalized_observation_satisfies_metric_valid_range() -> None:
    connector = CensusAcsConnector(retrieved_at=date(2026, 1, 1), api_key="test-key")
    with patch(
        "retirement_engine.connectors.census_acs.urlopen",
        return_value=_mock_response(SAMPLE_PAYLOAD),
