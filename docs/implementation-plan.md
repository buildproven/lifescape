...
Milestone 2 has begun: `retirement_engine.connectors.census_acs.CensusAcsConnector` implements the `Connector` protocol against the Census ACS 5-Year Data Profile API for the `education_attainment` metric, requiring a free `CENSUS_API_KEY` (see `.env.example`). It is not yet wired into `retire benchmark` or any CLI command — remaining Milestone 2 work is orchestrating connector runs into evidence ingestion, adding connectors for the other town-level metrics, and automated candidate discovery. Before wiring it in: `ACS_YEAR` is currently hardcoded to 2023 and must be bumped on each ACS release, and the state-FIPS lookup only covers the states in the current benchmark set — both need to be revisited alongside the freshness policy in `evidence.py`.
