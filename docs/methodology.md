...
A documented gate waiver permits ranking but does not fabricate evidence. If its critical metric remains missing, the affected criterion receives zero and the report marks the missing critical evidence explicitly.
...
Simulation count, seed, engine version, evaluation date, configuration, and evidence all participate in the run identity.
